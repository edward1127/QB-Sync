...
from google_sheet_model import TOKEN_SHEET
class AuthClient(requests.Session):
    ...
    def refresh(self, refresh_token=None):
        ...
        :raises `intuitlib.exceptions.AuthClientError`: if response status != 200
        """
        # get refresh_token from google sheet
        token  = refresh_token or TOKEN_SHEET.acell('A2').value  
        ...
        send_request('POST', self.token_endpoint, headers, self, body=urlencode(body), session=self)
        # update refresh_token and access token in google sheet
        TOKEN_SHEET.update_acell('A2', self.refresh_token)
        TOKEN_SHEET.update_acell('B2', self.access_token)
